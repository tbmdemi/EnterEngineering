...
class CoordinationLogicTest(unittest.TestCase):
    ...
    def test_evaluation_uses_stable_idempotency_key(self, ensure_task, upsert_evidence, _append_audit):
        ...
        with patch.object(module, "load_appointments", return_value=[
            {"id": "a", "chair": "C1", "starts_at": datetime(2026, 1, 1, 9, tzinfo=timezone.utc), "ends_at": datetime(2026, 1, 1, 10, tzinfo=timezone.utc), "status": "BOOKED"},
            {"id": "b", "chair": "C1", "starts_at": datetime(2026, 1, 1, 9, 30, tzinfo=timezone.utc), "ends_at": datetime(2026, 1, 1, 10, 30, tzinfo=timezone.utc), "status": "BOOKED"},
        ]):
            module.evaluate_coordination("e1", Role.FRONT_DESK)
            module.evaluate_coordination("e1", Role.FRONT_DESK)
        self.assertEqual([call.args[-1] for call in ensure_task.call_args_list], ["coord:e1:schedule-conflict"] * 2)
